...
- **Turn music** (post-launch follow-up): `audio.music` — a 16-step, 150 BPM two-voice chiptune loop (square bass + triangle pentatonic lead) that plays only in `performing`. `enterPerforming` calls `music.start()`; `repaintRing` calls `music.tick()` (lookahead scheduling off the existing 200 ms interval — no new registry timers); `transition()` calls `music.stop()` right after `wakeLock.sync`, cutting the scheduled tail through a per-turn gain node. Each note is gated by `settings.sound` at schedule time, so the corner mute works mid-turn; the step grid advances regardless, keeping the loop in phase.
